...
This is a Decoupled Settings template for creating contributed modules or themes. The project provides a complete development environment with CI configuration, testing setup, and deployment automation for publishing to Drupal.org.
...
- `decoupled_settings.*` - Template extension files
- `DecoupledSettingsService.php` - Main service class template
...
1. Run `php init.php` to customize template for Decoupled Settings
2. Build environment: `make build` or `ahoy build`
3. Develop Decoupled Settings code in `src/`
...
- SQLite database created in `/tmp/site_decoupled_settings.sqlite`
